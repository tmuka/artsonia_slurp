...
curl -s https://www.artsonia.com/slideshow.asp?artist=$ARTIST_ID | egrep 'src.*screenname' | sed "s;{src: '\(.*\)',screenname: '\(.*\)',artid: \(.*\),grade: '\(.*\)'}.*;curl -s \1 -o \"\2.\4.\3.artsonia.jpg\";g" | xargs -t -0 -i sh -c "{}"
